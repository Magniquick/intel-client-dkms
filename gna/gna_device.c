...
/* GNA is an inference accelerator, not a GPU, so it belongs on the accel
 * subsystem (/dev/accel/accelN) rather than as a DRM render node. Registering
 * as DRIVER_RENDER puts a /dev/dri/renderD* node in front of everything that
 * enumerates render nodes looking for a GPU. drivers/accel and
 * DRIVER_COMPUTE_ACCEL landed in v6.2; older kernels keep the render node.
 */
#ifdef GNA_HAVE_DRM_ACCEL
#include <drm/drm_accel.h>
DEFINE_DRM_ACCEL_FOPS(gna_drm_fops);
#define GNA_DRIVER_NODE_FEATURE DRIVER_COMPUTE_ACCEL
#else
DEFINE_DRM_GEM_FOPS(gna_drm_fops);
#define GNA_DRIVER_NODE_FEATURE DRIVER_RENDER
#endif
...
static void gna_pm_init(struct device *dev)
{
	...
	/* put_noidle() drops the probe-time reference without running an idle
	 * check, so nothing ever schedules the first autosuspend and the device
	 * sits in D0 until the first compute request happens to end with a
	 * put(). Observed: suspended 0 ms in 10 s after a modprobe, against
	 * ~99% suspended when the same driver was loaded at boot.
	 * put_autosuspend() drops the reference and queues the autosuspend.
	 */
...
static const struct drm_driver gna_drm_driver = {
	.driver_features = DRIVER_GEM | GNA_DRIVER_NODE_FEATURE,
	.open = gna_open,
	.postclose = gna_close,
	...
	.name = DRIVER_NAME,
	.desc = DRIVER_DESC,
#ifdef GNA_HAVE_DRM_DRIVER_DATE
	/* struct drm_driver::date removed by cb2e1c2136f7 ("drm: remove
	 * driver date from struct drm_driver and all drivers"), v6.14
	 */
	.date = DRIVER_DATE,
#endif
	.major = DRIVER_MAJOR,
...
static void gna_debugfs_drmm_fini(struct drm_device *drm, void *ptr)
{
	gna_debugfs_fini(ptr);
}
...
int gna_probe(struct device *parent, struct gna_dev_info *dev_info, void __iomem *iobase, int irq)
{
	...
	gna_priv->hw_info.hw_ver = FIELD_GET(GNA_IBUFFS_HW_VER, bld_reg);
	dev_dbg(parent, "GNA hardware version %u.%u (IBUFFS %#x)\n",
	...
	gna_debugfs_init(gna_priv);
	err = drmm_add_action_or_reset(drm_dev, gna_debugfs_drmm_fini, gna_priv);
	if (err)
		return err;
...
static u32 gna_device_type_by_hwid(u32 hwid)
{
	...
	case GNA_DEV_HWID_RPL_S:
		return GNA_DEV_TYPE_3_0;
	case GNA_DEV_HWID_MTL:
	case GNA_DEV_HWID_ARL:
	case GNA_DEV_HWID_ARL_S:
...
int gna_getparam(struct gna_device *gna_priv, union gna_parameter *param)
{
	...
	case GNA_PARAM_DEVICE_ID:
		...
		break;
	case GNA_PARAM_HW_VER:
		/* IBUFFS bits 31:24, as (major * 10 + minor). */
		param->out.value = gna_priv->hw_info.hw_ver;
		break;
	case GNA_PARAM_HAS_MMU:
		/* gna_mmu_init() runs unconditionally at probe. */
		param->out.value = 1;
		break;
	default:

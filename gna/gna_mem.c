...
bool gna_gem_object_put_pages_sgt(struct gna_gem_object *gnagemo)
{
	...
	kfree(shmem->sgt);
	shmem->sgt = NULL;
#ifndef GNA_HAVE_SHMEM_PUT_PAGES_UNLOCKED
	/* drm_gem_shmem_put_pages() became _locked() and now requires the
	 * caller to hold the reservation lock; release the pages before
	 * dropping it rather than after.
	 */

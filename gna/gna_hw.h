...
#define GNA_DEV_HWID_ARL	0x774C
#define GNA_DEV_HWID_ARL_S	0xAE4C
#define GNA_DEV_HWID_RPL_S	0x467E
...
/* Registers Intel's Windows driver (gna.sys 03.05.00.1578) uses that the
 * upstream Linux driver has no name for. Its ISR zeroes both when it sees
 * GNA_STS_SECONDARY (below). Their exact meaning is not documented; named
 * by offset deliberately rather than guessed at.
 */
#define GNA_MMIO_0xA0		0xA0
#define GNA_MMIO_0xA4		0xA4

/* IBUFFS bits 31:24 carry the GNA hardware version as (major*10 + minor):
 * 0x23 == GNA 3.5. The upstream driver masks IBUFFS to bits 7:0 and drops
 * this. Intel's Windows driver (gna.sys 03.05.00.1578) gates behaviour on
 * it being greater than 0x22, i.e. GNA 3.5 or newer.
 */
#define GNA_IBUFFS_HW_VER		GENMASK(31, 24)
#define GNA_HW_VER_3_5			0x23
...
/* gna.sys clears this when handling GNA_STS_SECONDARY; upstream never
 * touches bit 9 and has no name for it.
 */
#define GNA_CTRL_SECONDARY_INT_EN	BIT(9)
...
struct gna_hw_info {
	u8 in_buf_s;
	u8 hw_ver;	/* IBUFFS bits 31:24; 0x23 == GNA 3.5 */
};

...
/* GNA 3.5, per Intel's Windows gna.inf 03.05.00.1578. The hardware also
 * reports 0x23 (3.5) in IBUFFS bits 31:24.
 */
static const struct gna_dev_info arl_dev_info = {
	.hwid = GNA_DEV_HWID_ARL,
	GNA_GEN2_FEATURES
};

static const struct gna_dev_info arl_s_dev_info = {
	.hwid = GNA_DEV_HWID_ARL_S,
	GNA_GEN2_FEATURES
};

/* GNA 3.0 per the same INF. */
static const struct gna_dev_info rpl_s_dev_info = {
	.hwid = GNA_DEV_HWID_RPL_S,
	GNA_GEN2_FEATURES
};
...
static const struct pci_device_id gna_pci_ids[] = {
	...
	INTEL_GNA_DEVICE(GNA_DEV_HWID_ARL, &arl_dev_info),
	INTEL_GNA_DEVICE(GNA_DEV_HWID_ARL_S, &arl_s_dev_info),
	INTEL_GNA_DEVICE(GNA_DEV_HWID_RPL_S, &rpl_s_dev_info),
	{ }
};
...
static int __init gna_module_init(void)
{
	return pci_register_driver(&gna_pci_driver);
}
module_init(gna_module_init);

static void __exit gna_module_exit(void)
{
	pci_unregister_driver(&gna_pci_driver);
	gna_debugfs_exit();
}
module_exit(gna_module_exit);

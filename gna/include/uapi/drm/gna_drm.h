...
#include <drm/drm.h>  /* uapi drm.h; include/uapi is on the kernel include path */
...
/* Additions to reach parity with the parameter set Intel's Windows driver
 * exposes (GnaDrvApi.h: DEVICE_ID, HW_VER, HAS_MMU among others). Appended
 * so the existing four keep their values and current userspace is unaffected.
 * Not yet implemented, because I have no hardware source for them:
 * CE_NUM, PLE_NUM, AFE_NUM (engine counts) and QOS_HARD_TIMEOUT_MS.
 */
#define GNA_PARAM_DEVICE_ID		5
#define GNA_PARAM_HW_VER		6
#define GNA_PARAM_HAS_MMU		7
...
/* gna.sys handles these two; upstream defines neither. Bit 1 makes its ISR
 * clear GNA_CTRL_SECONDARY_INT_EN and zero MMIO 0xA0/0xA4. Bit 16 it simply
 * acknowledges, alongside saturation.
 */
#define GNA_STS_SECONDARY		_BITUL(1)
#define GNA_STS_ACK_16			_BITUL(16)

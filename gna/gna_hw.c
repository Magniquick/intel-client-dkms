...
int gna_abort_hw(struct gna_device *gna_priv)
{
	...
	/* On GNA 3.5 and newer the abort bit is self-clearing and must be
	 * seen to clear before the status register is meaningful. Intel's
	 * Windows driver (gna.sys 03.05.00.1578) polls CTRL here, gated on
	 * IBUFFS bits 31:24 being greater than 0x22, and treats a stuck bit
	 * as a failure. Older parts do not do this, so keep it version-gated
	 * rather than unconditional.
	 */
	if (gna_priv->hw_info.hw_ver >= GNA_HW_VER_3_5) {
		ret = readl_poll_timeout(gna_priv->iobase + GNA_MMIO_CTRL, val,
					 !(val & GNA_CTRL_ABORT_CLR_ACCEL),
					 0, 1000);
		if (ret) {
			dev_warn(gna_dev(gna_priv),
				 "abort not acknowledged, ctrl %#x\n", val);
			return ret;
		}
	}
